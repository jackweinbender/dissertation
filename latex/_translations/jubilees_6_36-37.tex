% !TEX root = dissertation.tex

%% COMPLETE
\begin{ethiopictext}
    \versenum{6:36}
    ወይከውኑ~፡ እለ~፡ ያስተሐይጹ~፡ ወርኀ~፡
    በሑያጼ~፡ ወርኀ~። እስመ~፡ ትማስን~፡ ይእቲ~፡ ጊዜያተ~፡ ወትቀድም~፡
    እምዓመታት~፡ ለዓመት~፡ ዐሡረ~፡ ዕለተ~።
    \versenum{37}
    በእንተዝ~፡ ይመጽእ~፡
    ዓመታተ~፡ ሎሙ~፡ እንዘ~፡ ያማስኑ~፡ ወይገብሩ~፡ ዕለተ~፡ ስምዕ~፡
    ምንንተ~። ወዕለተ~፡ ርኵስተ~፡ በዓለ~፡ ወኵሉ~፡ ይዴምር~፡ ወመዋዕለ~፡ 
    ቅዱሳተ~፡ ርኩሰ~፡ ወዕለተ~፡ ርኵስተ~፡ ለዕለት~፡ ቅድስት~። እስመ~፡
    ይስሕቱ~፡ አውራኀ~፡ ወሰንበታተ ወብዓላተ~፡ ወኢዩቤለ~።
\end{ethiopictext}
\begin{transliteration}
    \versenum{6:36}
    wa-yəkawwənu ʔəlla yāstaḥayyəṣu warḫa 
    % yekawwenu     G impf. √kwn 'to be' 
    % yāstaḥayyeṣu  CGt impf. √ḥyṣ 'to perceive, observe (closely)' (Les. 252) 
    % warḫa         warḥ pl. ʔawrāḫa 'moon, month' (Les. 617) 
    ba-ḥuyāṣe warḫ ʔəsma təmās(s)ən yəʔəti gizeyāta wa-təqaddəm 
    % ḥuyāṣe        'observation'
    % esma          'because'
    % temās(s)en    L impf. √msn 'decay, corrupt' (Les. 366)
    % yeʔeti        'she, it' (here, the moon)
    % gizeyāta      gize pl. gizeyāt 'time, season' (Les. 210)
    % teqaddem      G impf. 3fs √qdm 'to precede, go before'
    ʔəm-ʕāmatāt la-ʕāmat ʕaśura ʕəlata
    % ʕāmatāt       ʕām pl. ʕāmatāt 'year' (Les. 62)
    % ʕaśur         'the tenth day, ten days' (Les. 73)
    % ʕelata        'day' (Les. 603)
    \versenum{37}
    ba-ʔənta-zə yəmaṣṣəʔ 
    % yemaṣṣeʔ          g impf 3ms √mṣʔ 'come, happen, arise, overtake'
    ʕāmatāta lomu ʔənza yāmās(s)ənu wa-yəgabbəru ʕəlata səmʕ 
    % yāmās(s)enu       CL impf 3mp √msn 'decay, be corrupt' Les. 366
    % yegabberu         g impf 3mp √gbr 'act, do'
    % semʕ              √smʕ 'rumor, news, witness, testimony'
    mənnənta wa-ʕəlata rə\kw{ə}sta baʕāla wa-\kw{ə}llu yədemmər wa-mawāʕəla 
    % mennenta           D √mnn 'despise, reject, renounce'
    % rekwest           adj. √rkws 'to be unclean, impure' Les. 470
    % baʕāla            'festival' Les. 83
    % yedemmer          D impf. √dmr 'to insert, add, mix, mingle, multiply (arithmetic)
    qəddusāta rəkusa wa-ʕəlata rə\kw{ə}sta la-ʕəlat qəddəst ʔəsma
    % rekusa            adj. √rkws 'to be unclean, impure' Les. 470
    % laʕlat            
    yəsəḥḥətu ʔawrāḫa wa-sanbatāta wa-baʕālāta wa-ʔiyyobela
    % yeseḥetu          √sḥt 'wound, harm, violate'
    % ʔewrāḫa           'months' warḥ op. cit.
    % sanbatāta         'sabbaths'
\end{transliteration}
\begin{translation}
    \versenum{6:36}
    There will be those who watch the moon closely with lunar observations
    because it is deficient (concerning) the seasons and is premature from year to year by ten days. 
    \versenum{37}
    Therefore
    years will come about for them when they decay. And they will make a day of
    testimony despised and a profane day a festival. All will mingle holy days
    (with) the profane and a profane day with a holy day, for
    they will err in months and sabbaths and festivals and the jubilee.
\end{translation}