...
Unsurprisingly, Najman's approach to \jub and Mosaic Discourse dovetails quite well with the idea of social and cultural memory theory. The way that Najman describes the growth and development of the Author extending beyond the historical and literary bounds of the ``real'' author is evocative of the process to that of memory construction. In fact, from my perspective, what Najman describes \emph{is} a process of memory construction, though she does not use the terminology. What she describes as Mosaic Discourse is the same set of processes which enabled the author(s) of the Enochic works to expand on and speculate about the Watchers and the Flood, and which enabled the \ga to draw from those traditions in its rewriting.%
    \footnote{The \ga, of course, may have also drawn from the book of \jub, which only goes to further this point.}
Given fascination with the figure of Enoch in the \secondtemple period, we could just as easily talk about ``Enochic Discourse'' when we discuss the various and sundry texts which draw from, expand, and reframe the enigmatic antedeluvian figure. Furthermore, we can easily identify additional Discourses about the figures of Abram, Daniel, and David, all of whom are the subjects of expanding bodies of literary production in the \secondtemple period, albeit not all as \psa, and not all with the same foundational significance as that of Moses.

% ALL OF THIS IS MEMORY

% WHAT IS INTERESTING ABOUT JUB is the posture toward MEMORY: PRESCRIPTIVE, PRACTICE

% Making memory + constructing authority -> Practical Outcomes
% Memory constriction is not only an academic affair---constructed
% memory operates within frameworks, but it also creates new frames of reference which impact *practice*.

% Give something small as an example, set the stage for the calendar
